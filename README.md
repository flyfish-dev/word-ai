...
This installs the Python MCP server, builds the Office.js taskpane, builds the .NET Open XML engine when .NET SDK 8 is available, writes `.wordai/codex-config.toml`, and optionally installs the `word-ai` Codex Skill to `~/.codex/skills/word-ai`.
...
安装脚本会安装 Python MCP 依赖、构建 Office.js taskpane、在可用时构建 .NET Open XML 引擎、生成 `.wordai/codex-config.toml`，并可把正式 `word-ai` Codex Skill 安装到 `~/.codex/skills/word-ai`。
