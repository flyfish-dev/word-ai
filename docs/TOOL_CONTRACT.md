# MCP 工具契约 v0.4：结构稳定优先

本契约面向 Codex / OpenAI Agents / MCP 客户端。设计目标是：让模型拥有足够丰富的读取、定位和规划能力，但把所有正式 DOCX 写入都收口到受控 PatchSet 事务中。

## 总原则

1. **读工具丰富，写工具收口**：Codex 可以广泛读取结构、索引、锚点、表格、字段、批注和修订信息；正文写入只能通过 `docx_apply_patchset`。
...
Office taskpane 使用 `word_ai_mcp.server_http` 的 `/office/*` REST 封装，但底层仍调用本契约中的 MCP tools：
...
Bridge 不新增绕过 PatchSet 的文件写入口；所有文件级 DOCX 正文修改仍由 `docx_apply_patchset` 完成。`Apply Open` 只作用于当前 Word 会话中的内容控件文本，并要求 `expected_old_sha256` 与打开文档当前文本一致。

## 工具清单

当前 `tools/list` 暴露 **49 个工具**，包含常用别名，便于 Codex 以自然名称调用。
...
- 优先内容控件 tag。
- 重要写入必须先 `docx_assess_patchset` 和 `docx_dry_run_patchset`。
- 关键交付内容、段落兜底操作、表格结构操作、批注和锚点治理必须带 `source_sha256` 和目标级 `expected_old_sha256`。
- 不允许全量重建 DOCX。
