...
GitHub Release 还会包含每个 RID 的 native 压缩包：`word-ai-openxml-<version>-<rid>.tar.gz` 或 `.zip`，standalone 二进制 `word-ai-standalone-<version>-<rid>`，quickstart 包 `word-ai-quickstart-<version>-<rid>`，以及 checksum 文件。
...
7. Standalone workflow 会构建并烟测 `linux-x64`、`linux-arm64`、`osx-arm64`、`osx-x64`、`win-x64` 和 `win-arm64` 的单文件二进制与 quickstart 包，并上传到同一个 GitHub Release。
