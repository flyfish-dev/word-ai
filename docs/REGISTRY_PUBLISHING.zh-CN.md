...
```bash
scripts/publish_dotnet.sh osx-arm64      # CI 中对每个 release RID 重复执行
...
如果构建 MCPB 前已经存在 `dist/native/<rid>/WordAi.OpenXml`，native .NET Open XML 后端会被打包，并优先于 DLL、项目和 Python fallback 被选择。MCPB bootstrap 仍要求 Python 3.10+ 运行 MCP facade；首次启动会在安装后的 bundle 目录内创建 `.word-ai-mcpb-venv`，安装 Word AI 依赖，然后启动 stdio MCP server。Bootstrap 安装日志写入 stderr，stdout 保留给 MCP JSON-RPC。
...
4. GitHub Actions 为目标 RID 发布 native .NET 后端，构建 `word-ai-<version>.mcpb`，并计算 `fileSha256`。
5. 工作流上传 MCPB 文件到 GitHub Release。
6. 工作流通过 GitHub OIDC 登录 MCP Registry。
7. `mcp-publisher validate` 和 `mcp-publisher publish` 基于 `server.json` 发布版本元数据。
